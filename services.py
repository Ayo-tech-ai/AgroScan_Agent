...
class FarmRecordService:
    """
    Provides all business operations for the farm record book.
    """
    ...
    def get_connection(self):
        """
        Return a connection to the farm database.
        """
...
def validate_daily_record(self, bird_count, crates_collected):
        """
        Validate that the reported egg production is realistic.
        """
    ...
    def record_daily_farm_data(
        ...
        """
        Record or update a daily farm record.

        Missing values inherit from today's existing record first,
        then from the most recent previous record.
        """
        ...
        # Use today's record first (if it exists), otherwise inherit
        # from the most recent earlier record.
        ...
        if reference_record:
            ...
            if crates_collected is None:
                raise ValueError(
                    "crates_collected is required for the first record."
                )

            if bird_count is None:
                raise ValueError(
                    "bird_count is required for the first record."
                )

            if feed_consumed_kg is None:
                raise ValueError(
                    "feed_consumed_kg is required for the first record."
                )

            if expenses is None:
                raise ValueError(
                    "expenses is required for the first record."
                )

        revenue = crates_collected * self.CRATE_PRICE

        valid, message = self.validate_daily_record(
            bird_count,
            crates_collected,
        )
...
"""
Helper functions used by the FunctionTools.

Some language models (especially through LiteLLM/Groq)
may pass numeric values as strings. These helpers safely
convert them before the business logic executes.
"""
...
def _to_int(value, field_name):
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            raise ValueError(
                f"Could not interpret '{value}' "
                f"as a whole number for {field_name}."
            )
